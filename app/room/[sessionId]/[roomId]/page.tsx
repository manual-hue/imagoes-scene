...
import { markRoomVisited } from '@/lib/visited';
import type { RoomSceneClue, RoomSceneHotspot } from '@/types/room';
...
export default function RoomPage() {
  ...
  const scene = room ? getRoomScene(room.id) : null;
  const [selectedClue, setSelectedClue] = useState<RoomSceneClue | null>(null);

  useEffect(() => {
    if (room) {
      markRoomVisited(params.sessionId, params.roomId, room.name);
    }
  }, [room, params.sessionId, params.roomId]);
  ...
  return (
    <RoomBackground room={room}>
      <div className="flex h-full flex-col">
        ...
        <BottomTabBar sessionId={params.sessionId} roomId={params.roomId} />
      </div>
